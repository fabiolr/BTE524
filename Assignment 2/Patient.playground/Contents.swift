...
// Then, list all upcoming visits by doctor, only for doctors with the specialty of GENERAL_MEDICINE.


for doctor in doctors where doctor.specialty == Specialty.General_Medicine {
    
        print("Doctor: \t\t\t\t\t \((doctor.name))")
        print("Specialty: \t\t\t\t \(doctor.specialty!)")
        print("Upcoming Visits:")
    
        for appointment in schedule {
            
                if doctor.name == appointment[1] {
                    ...
                    print("\tVisit Date::\t\t\t \(appointment[2])")
                    print("\tFirst Name:\t\t\t \(splittedPatient[0])")
                    print("\tLast Name:\t\t\t\t \(splittedPatient[1])")
...
// list all upcoming visits by patient, only for patients that actually have an upcoming visit, to the console output
...
// need to do other 2 reports,  basically the same logic..
